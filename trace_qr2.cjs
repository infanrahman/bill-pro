...
async function run() {
    ...
    // In actual app, we do `cleanUpCertificateString` implicitly by `.replace(/\\s+/g, '')`.
    // But library does `.replace("-----BEGIN...", "")` leaving newlines inside.
    const cleanCert = certPem.replace(/-----BEGIN CERTIFICATE-----/g, '').replace(/-----END CERTIFICATE-----/g, '').trim();
    const cleanKey = keyPem.replace(/-----BEGIN EC PRIVATE KEY-----/g, '').replace(/-----END EC PRIVATE KEY-----/g, '').trim();
