    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                {/* Header */}
                <div className="bg-gradient-to-r from-blue-900 to-slate-900 p-6 flex justify-between items-center border-b border-slate-800">
                ...
                <div className="p-6">
                    {success ? (
                        <div className="text-center py-8">
                            ...
                            <p className="text-slate-400">{t('license.thank_you', 'Thank you for your purchase.')}</p>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700 mb-4">
                                <div className="flex justify-between items-center mb-2">
                                    <p className="text-sm text-slate-400">{t('license.machine_id', 'Machine ID')}:</p>
                                    <button
                                        type="button"
                                        onClick={handleCopy}
                                ...
                                <code className="block bg-black/50 p-3 rounded text-blue-300 font-mono text-sm break-all select-all border border-slate-800/50">
                                    {machineId || 'Loading...'}
                                </code>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-3 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono tracking-wide"
                                    required
                                />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-lg transition-all shadow-lg hover:shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center"
                            >
                                {loading ? (
...
export const TrialBanner: React.FC = () => {
    ...
    return (
        <>
            <div className={`${bgColor} text-white px-4 py-1.5 flex justify-between items-center text-sm font-medium shadow-md relative z-50`}>
                <div className="flex items-center gap-2">
                    <span className="bg-white/20 px-1.5 py-0.5 rounded text-xs font-bold uppercase tracking-wider">
                ...
                <button
                    ...
                    className="bg-white text-indigo-900 hover:bg-indigo-50 px-3 py-1 rounded text-xs font-bold transition-colors shadow-sm"
                >
