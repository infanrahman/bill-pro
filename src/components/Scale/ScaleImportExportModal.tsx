...
export const ScaleImportExportModal: React.FC<ScaleImportExportModalProps> = ({ isOpen, onClose, currentRows, onImport, onExport }) => {
 ...
 addToast("Empty file or unrecognized format.","error");
 }
 } catch (err) {
 console.error("Error parsing file", err);
 addToast("Failed to parse the file. Please ensure it is a valid Excel or CSV file.","error");
 }
 };
 ...
 const rawValue = mappedHeader && dataRow[mappedHeader] !== undefined && dataRow[mappedHeader] !== null
 ...
 <div className="bg-white dark:bg-slate-800 rounded-xl w-full max-w-4xl overflow-hidden flex flex-col h-[85vh] max-h-[800px]">
 {/* Header */}
 ...
 <p className="text-sm text-slate-700 dark:text-slate-300">Map your Excel columns to the Scale Database fields.</p>
 </div>
 <button type="button"onClick={onClose} className="p-2 text-slate-600 hover:text-slate-600 dark:hover:text-slate-200">
 <X size={20} />
 </button>
 </div>
 ...
 className="w-full py-3 bg-slate-900 dark:bg-white hover:bg-slate-900 dark:hover:bg-white text-white rounded-xl font-bold"
 >
 ...
 <div className="relative flex justify-center text-sm"><span className="px-2 bg-slate-50 dark:bg-slate-900 text-slate-700">OR</span></div>
 </div>

 <button type="button"
 onClick={handleExport}
 className="w-full py-3 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl font-bold"
 >
 ...
 <div className="mb-4 flex items-center gap-3 p-3 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg">
 <AlertCircle size={20} />
 ...
 <tr key={field.key} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800">
 ...
 className="w-full h-9 px-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-md outline-none focus:ring-2 focus:ring-slate-900/20 dark:focus:ring-white/20 text-sm dark:text-white"
 >
 ...
 value={defaultValues[field.key]}
 onChange={(e) => setDefaultValues({ ...defaultValues, [field.key]: e.target.value })}
 className="w-full h-9 px-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-md outline-none focus:ring-2 focus:ring-slate-900/20 dark:focus:ring-white/20 text-sm dark:text-white disabled:opacity-50 disabled:bg-slate-100"
 disabled={!!fieldMapping[field.key]}
 />
)}
 </td>
 ...
 {/* Footer */}
 <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 flex justify-between">
 <button type="button"
 onClick={() => phase === 'map' ? setPhase('select') : onClose()}
 className="px-5 py-2.5 text-slate-600 dark:text-slate-300 font-medium hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg"
 >
 ...
 className="px-6 py-2.5 bg-slate-900 dark:bg-white text-white font-medium hover:bg-slate-900 dark:hover:bg-white rounded-lg flex items-center gap-2"
 >
 <Save size={18} />
