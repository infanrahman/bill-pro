...
const PurchaseDetailsModal: React.FC<PurchaseDetailsModalProps> = ({ purchase, isOpen, onClose }) => {
 ...
 maxWidth="4xl"
 >
 <div className={`flex-1 overflow-y-auto p-8 bg-white dark:bg-slate-900 text-slate-900 dark:text-white ${isRTL ? 'rtl' : 'ltr'}`} style={{ fontFamily: 'Segoe UI, sans-serif' }}>
 ...
 */}
 ...
 <td className={`p-3 text-slate-700 ${isRTL ? 'text-left' : 'text-right'}`}>{formatCurrency(calculatedTax)}</td>
