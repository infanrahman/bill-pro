    ...
    if (!hasPermission('settings_manage')) {
        return (
            <div className="flex flex-col items-center justify-center h-screen text-center p-8">
                ...
                <p className="text-slate-500">{t('settings.access_denied_msg') || 'You do not have permission to manage business settings.'}</p>
            </div>
        );
    }
        ...
        <div className="p-6 max-w-4xl mx-auto space-y-8 pb-20">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex items-center gap-4 mb-4">
                        <div className="w-20 h-20 rounded-lg border border-slate-300 dark:border-slate-600 flex items-center justify-center overflow-hidden bg-slate-50 dark:bg-slate-900">
                            {details.logoUrl ? (
                                ...
                                <span className="text-xs text-slate-400">{t('settings.no_logo')}</span>
                            )}
                        </div>
                        <div>
                    ...
                    <button
                        type="submit"
                        className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-semibold transition-colors"
                    >
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <form onSubmit={handleAddUser} className="space-y-4">
                            <h3 className="text-sm font-medium text-slate-500 uppercase">{t('settings.add_shopkeeper')}</h3>
                            <div>
                                <input
                                    type="text"
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                    ...
                                    <div key={user.id} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
                                        <div>
                                            ...
                                            <div className="text-xs text-slate-500 flex flex-col gap-1">
                                                ...
                                                <span className={user.permissions?.includes('backup') ? "text-green-600" : "text-slate-400"}>
                                ...
                                {users?.length === 0 && (
                                    <p className="text-slate-400 text-sm italic">{t('settings.no_users')}</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mt-8 border-t border-slate-200 dark:border-slate-700 pt-6">
                        <h3 className="text-sm font-medium text-slate-500 uppercase mb-4">{t('settings.change_admin_pass')}</h3>
                        <ChangeAdminPassword />
                    </div>
                </div>
            )}
