            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/50 min-h-[400px] overflow-hidden">
                        ...
                        <div className="md:hidden space-y-3 p-4">
                            {!currentList ? (
                                Array.from({ length: 3 }).map((_: any, i: any) => (
                                    <div key={i} className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700 shadow-sm animate-pulse">
                                ...
                                currentList.map((invoice: any) => (
                                    <div key={invoice.id} className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700 shadow-sm">
                                        <div className="flex items-start justify-between mb-2">
                                            <div>
                                                <div className="font-bold text-sm text-slate-900 dark:text-white">{invoice.invoiceNumber || '-'}</div>
                                                <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{invoice.customerName || 'Unknown'}</div>
                                            </div>
                                            <span className={clsx(
                                        ...
                                        <div className="text-xs text-slate-500 space-y-1 mb-3">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-medium text-slate-700 dark:text-slate-300">{formatDate(invoice.createdAt)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>{t('sales.amount')}</span>
                                                <span className="font-bold text-base text-slate-900 dark:text-white">{formatCurrency(invoice.grandTotal)}</span>
                                            </div>
                                            {activeTab === 'return' && isZatcaEnabled && (
                                                <div className="flex justify-between items-center">
                                                    <span>ZATCA</span>
                                                    {invoice.zatcaStatus === 'REPORTED' ? (
                                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400">
                                                            <ShieldCheck size={10} /> Reported
                                                        </span>
                                                    ) : invoice.zatcaStatus === 'ERROR' ? (
                                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase bg-rose-50 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400">
                                                            <ShieldAlert size={10} /> Error
                                                        </span>
                                                    ) : (
                                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                                        ...
                                        <div className="flex gap-2 pt-3 border-t border-slate-100 dark:border-slate-700">
                                            {activeTab === 'order' && invoice.status === 'pending' && (
                                                <button
                                                        ...
                                                        try {
                                                            await db.transaction('rw', [db.invoices, db.items, db.customers], async () => {
                                                                ...
                                                                if (invoice.customerId) {
                                                                    ...
                                                                    if (customer) {
                                                                        await db.customers.update(invoice.customerId, { balance: (customer.balance || 0) + invoice.remainingAmount });
                                                                    }
                                                                }
                                                            });
                                                    ...
                                                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 text-xs font-medium"
                                                    title={t('sales.convert_invoice')}
                                                >
                                                    <RotateCcw size={13} className="rotate-180" /> {t('sales.convert_invoice')}
                                                </button>
                                            )}
                                            <button
                                                onClick={() => printInvoice(invoice)}
                                                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-medium"
                ...
                {activeTab === 'payment' && (
                    <div className="overflow-x-auto">
                        <div className="p-4 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 flex flex-col gap-3 md:flex-row md:flex-wrap md:items-end md:justify-between">
