        ...
        <div className="h-full flex flex-col space-y-2">
            <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-2 flex-1">
                    <button onClick={onBack} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300">
                        <ArrowLeft size={20} />
                    </button>
                    <input
                        type="text"
                        ...
                        className="font-bold text-lg bg-transparent border-none outline-none focus:ring-1 ring-blue-500 rounded px-1 dark:text-white"
                ...
                <div className="flex items-center gap-2">
                    <div className="flex gap-1 border-l pl-2 border-slate-200 dark:border-slate-700">
                        <button onClick={saveState} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300 flex items-center gap-2" title="Save">
                            <Save size={18} />
                            {isSaving && <span className="text-xs">Saving...</span>}
                        </button>
                        <button onClick={undo} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300" title="Undo"><Undo size={18} /></button>
                        <button onClick={redo} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300" title="Redo"><Redo size={18} /></button>
                        <div className="w-px bg-slate-200 dark:bg-slate-700 mx-1" />
                        <button onClick={() => toggleStyle('bold', true)} className={`p-2 rounded ${styles[`${selectedCell?.r},${selectedCell?.c}`]?.bold ? 'bg-blue-100 text-blue-600' : 'hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300'}`}><Bold size={18} /></button>
                        <button onClick={() => toggleStyle('align', 'left')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300"><AlignLeft size={18} /></button>
                        <button onClick={() => toggleStyle('align', 'center')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300"><AlignCenter size={18} /></button>
                        <button onClick={() => toggleStyle('align', 'right')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300"><AlignRight size={18} /></button>
                        <div className="w-px bg-slate-200 dark:bg-slate-700 mx-1" />
                        ...
                        <div className="w-px bg-slate-200 dark:bg-slate-700 mx-1" />
                        <button onClick={() => toggleStyle('border', true)} className={`p-2 rounded ${styles[`${selectedCell?.r},${selectedCell?.c}`]?.border ? 'bg-blue-100 text-blue-600' : 'hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300'}`}><Grid3X3 size={18} /></button>
                        <button onClick={() => toggleStyle('wrapText', true)} className={`p-2 rounded ${styles[`${selectedCell?.r},${selectedCell?.c}`]?.wrapText ? 'bg-blue-100 text-blue-600' : 'hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300'}`}><WrapText size={18} /></button>
                    </div>
                    <button onClick={exportExcel} className="p-2 hover:bg-green-100 text-green-600 rounded flex items-center gap-1">
                        <Download size={18} /> <span className="text-xs font-bold">Export</span>
                    </button>
                    <button onClick={() => setShowHelp(true)} className="p-2 text-slate-400 hover:text-blue-600"><HelpCircle size={18} /></button>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                <div className="font-bold text-xs text-slate-500 w-8 text-center bg-slate-100 dark:bg-slate-700 rounded py-1">
            ...
            {/* Grid */}
            <div className="flex-1 overflow-auto bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shadow-inner" onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
                        ...
                        <tr>
                            ...
                            {headers.map((h, i) => (
                                <th key={i} className="border border-slate-200 dark:border-slate-700 p-0 relative group" style={{ width: colWidths[i] || 100 }}>
                                    ...
                                    />
                                    <div
                                        className="absolute right-0 top-0 w-1 h-full cursor-col-resize hover:bg-blue-400 z-40"
                                        onMouseDown={(e) => {
                                            e.stopPropagation();
                                            setResizingCol(i);
                            ...
                            <th className="w-8 border border-slate-200 dark:border-slate-700 p-0">
                                <button onClick={() => addCol()} className="w-full h-full hover:bg-slate-200 flex items-center justify-center text-slate-400"><Plus size={14} /></button>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.map((row, r) => (
                            <tr key={r} style={{ height: rowHeights[r] || 24 }}>
                                ...
                                >
                                    {r + 1}
                                    <div
                                        className="absolute bottom-0 left-0 w-full h-1 cursor-row-resize hover:bg-blue-400 z-40"
                                        onMouseDown={(e) => {
                                            e.stopPropagation();
                                            setResizingRow(r);
                                ...
                                {row.map((cell, c) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {isSelected && (
                                                <div
                                                    className="absolute bottom-[-4px] right-[-4px] w-3 h-3 bg-blue-500 cursor-crosshair z-20 border-2 border-white"
                                                    onMouseDown={(e) => {
                                                        e.stopPropagation();
                                                        setIsFilling(true);
                ...
                <div className="p-2">
                    <button onClick={() => addRow()} className="flex items-center gap-1 text-xs font-bold text-blue-600 uppercase tracking-wider hover:bg-blue-50 p-2 rounded">
...

            {
                contextMenu && (
                    <div
                        className="fixed bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl rounded py-1 z-50 text-sm"
                        style={{ top: contextMenu.y, left: contextMenu.x }}
                    >
                        <button onClick={() => { handleCut(); setContextMenu(null); }} className="block w-full text-left px-4 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-white flex items-center gap-2"><Scissors size={14} /> Cut</button>
                        <button onClick={() => { handleCopy(); setContextMenu(null); }} className="block w-full text-left px-4 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-white flex items-center gap-2"><Copy size={14} /> Copy</button>
                        <button onClick={() => { handlePaste(); setContextMenu(null); }} className="block w-full text-left px-4 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-white flex items-center gap-2"><Clipboard size={14} /> Paste</button>
                        <div className="h-px bg-slate-200 dark:bg-slate-700 my-1" />
                        <button onClick={() => { addRow(contextMenu.r); setContextMenu(null); }} className="block w-full text-left px-4 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-white flex items-center gap-2" disabled={contextMenu.r === -1}><Plus size={14} /> Insert Row</button>
                        <button onClick={() => { addCol(contextMenu.c); setContextMenu(null); }} className="block w-full text-left px-4 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-white flex items-center gap-2" disabled={contextMenu.c === -1}><Plus size={14} /> Insert Column</button>
                        <div className="h-px bg-slate-200 dark:bg-slate-700 my-1" />
                        <button onClick={() => { clearFormatting(); setContextMenu(null); }} className="block w-full text-left px-4 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-white flex items-center gap-2"><Eraser size={14} /> Clear Content</button>
                        <div className="h-px bg-slate-200 dark:bg-slate-700 my-1" />
                        <button onClick={() => { deleteRow(); setContextMenu(null); }} className="block w-full text-left px-4 py-1.5 hover:bg-red-50 text-red-600 flex items-center gap-2"><Minus size={14} /> Delete Row</button>
                        <button onClick={() => { deleteCol(); setContextMenu(null); }} className="block w-full text-left px-4 py-1.5 hover:bg-red-50 text-red-600 flex items-center gap-2"><Trash2 size={14} /> Delete Column</button>
                    </div>
                )
            }

            {/* Status Bar */}
            <div className="bg-slate-100 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 p-1 flex justify-end gap-6 text-xs text-slate-600 dark:text-slate-400 font-mono px-4">
                {stats ? (
                    <>
