...
const ScaleTab: React.FC = () => {
 ...
 className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-900 dark:text-indigo-400 rounded-lg font-medium text-sm disabled:opacity-50"
 >
 <RefreshCw size={16} className={isBulkSyncing ? '' : ''} />
 {t('scales.bulk_update', { defaultValue: 'Bulk Update All' })}
 </button>
 <button type="button"
 onClick={() => setIsAdding(!isAdding)}
 className="flex items-center gap-2 px-4 py-2 bg-slate-900 dark:bg-white text-white hover:bg-slate-900 dark:hover:bg-white rounded-lg font-medium text-sm"
 >
 <Plus size={16} />
 ...
 className="px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-slate-900/20 dark:focus:ring-white/20 dark:text-white"
 ...
 className="flex-1 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-slate-900/20 dark:focus:ring-white/20 dark:text-white"
 ...
 disabled={isScanning}
 className="px-3 py-2 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-900 dark:text-indigo-400 rounded-lg flex items-center justify-center disabled:opacity-50"
 title="Auto-Detect Scale on Network"
 >
 {isScanning ? <RefreshCw size={18} className=""/> : <Search size={18} />}
 </button>
 </div>
 </div>
 ...
 <button
 type="submit"
 className="px-4 py-2 bg-slate-900 dark:bg-white text-white hover:bg-slate-900 dark:hover:bg-white rounded-lg"
 >
 {t('common.save')}
 </button>
 </div>
 ...
 <div key={scale.id} className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
 <div className="flex items-center gap-4">
 <div className={`p-3 rounded-full ${scale.status === 'online' ? 'bg-green-100 text-green-600 dark:bg-green-900/30' : scale.status === 'offline' ? 'bg-red-100 text-red-600 dark:bg-red-900/30' : 'bg-slate-100 text-slate-600 dark:bg-slate-700'}`}>
 <Server size={24} />
 </div>
 <div>
 ...
 <div className="text-sm text-slate-700 dark:text-slate-300 mt-1 flex items-center gap-3">
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg disabled:opacity-50"
 >
 <Wifi size={16} className={isTesting === scale.id ? ' text-slate-900 dark:text-white' : ''} />
 {t('scales.test_config', { defaultValue: 'Test' })}
 </button>
 <button type="button"
 onClick={() => handleOpenManager(scale)}
 disabled={isSyncing === scale.id}
 className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:hover:bg-purple-900/50 dark:text-purple-400 text-sm font-medium rounded-lg disabled:opacity-50"
 >
 <Settings size={16} />
 Manager
 </button>
 ...
 disabled={isSyncing === scale.id}
 className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:hover:bg-amber-900/50 dark:text-amber-400 text-sm font-medium rounded-lg disabled:opacity-50"
 >
 <Download size={16} />
 ...
 disabled={isSyncing === scale.id}
 className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:hover:bg-emerald-900/50 dark:text-emerald-400 text-sm font-medium rounded-lg disabled:opacity-50"
 >
 <Keyboard size={16} />
 Hotkeys
 </button>
 ...
 disabled={isSyncing === scale.id}
 className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:hover:bg-indigo-900 dark:text-indigo-400 text-sm font-medium rounded-lg disabled:opacity-50"
 >
 {isSyncing === scale.id ? <RefreshCw size={16} className=""/> : <Server size={16} />}
 Time Sync
 </button>
 <button type="button"
 onClick={() => handleIncrementalSync(scale)}
 disabled={isSyncing === scale.id}
 className="flex items-center gap-1.5 px-3 py-1.5 bg-sky-50 hover:bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:hover:bg-sky-900/50 dark:text-sky-400 text-sm font-medium rounded-lg disabled:opacity-50"
 >
 {isSyncing === scale.id ? <RefreshCw size={16} className=""/> : <Upload size={16} />}
 Upload Selected
 </button>
 <button type="button"
 onClick={() => handleFullSync(scale)}
 disabled={isSyncing === scale.id}
 className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-900 dark:text-white text-sm font-medium rounded-lg disabled:opacity-50"
 >
 {isSyncing === scale.id ? <RefreshCw size={16} className=""/> : <Upload size={16} />}
 Full Sync
 </button>
 ...
 disabled={isSyncing === scale.id}
 className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 dark:bg-red-900/30 dark:hover:bg-red-900/50 dark:text-red-400 text-sm font-medium rounded-lg disabled:opacity-50 ml-2"
 ...
 className="p-1.5 text-slate-600 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg ml-2"
 ...
 <Server size={48} className="mx-auto text-slate-300 dark:text-slate-300 mb-4"/>
 <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-1">No scales configured</h3>
 <p className="text-slate-700 max-w-sm mx-auto mb-4">Add your network IP weighing scales to begin syncing items and PLUs.</p>
 <button type="button"
 onClick={() => setIsAdding(true)}
 className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-white rounded-lg font-medium text-sm"
 >
 <Plus size={16} />
 Add First Scale
 </button>
 ...
 <div className="bg-white dark:bg-slate-800 rounded-xl w-full max-w-sm overflow-hidden p-6 text-center zoom-in-95">
 ...
 className="px-5 py-2.5 text-slate-700 bg-slate-100 hover:bg-slate-200 dark:text-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-lg font-semibold w-full"
 >
 {t('common.cancel', { defaultValue: 'Cancel' })}
 </button>
 <button type="button"
 onClick={confirmDeleteScale}
 className="px-5 py-2.5 bg-red-600 text-white hover:bg-red-700 rounded-lg font-semibold w-full"
 >
