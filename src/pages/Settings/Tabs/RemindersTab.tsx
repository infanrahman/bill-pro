...
const RemindersTab: React.FC = () => {
 ...
 <button type="button"
 onClick={onChange}
 className={`w-14 h-8 rounded-full relative ${enabled ? 'bg-slate-900 dark:bg-white' : 'bg-slate-300 dark:bg-slate-600'}`}
 >
 <span className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full ${enabled ? 'translate-x-6' : 'translate-x-0'}`} />
 </button>
);
 ...
 <Bell className="text-slate-900 dark:text-white"/> {t('reminders.title')}
 </h2>
